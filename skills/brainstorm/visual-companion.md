...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.claude/brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add `.claude/brainstorm/` to `.gitignore` if it's not already there.
...
If the session used `--project-dir`, mockup files persist in `.claude/brainstorm/` for later reference. Only `/tmp` sessions get deleted on stop.
